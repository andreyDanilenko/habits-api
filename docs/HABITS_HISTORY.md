...
| Источник | Роль |
|----------|------|
| **habits** | **Сегодня и будущее** — основной источник. Список привычек на дату D >= сегодня берётся напрямую из `habits` (расписание, created_at <= D). |
| **habit_versions** | **Прошлое** — историческая точность. Привычки на дату D < сегодня: версии с `valid_from <= D <= valid_to` и расписание на D. |
| **habit_completions** | **Прошлое** — дополняет versions. Привычки с выполнением в день D, которых нет в versions (сироты), добавляются из versions или habits. |

Итог: для **сегодня и будущего** — только `habits`. Для **прошлого** — `habit_versions` + `habit_completions` (сироты).
...
**Фолбэк на таблицу `habits`** выполняется для **любых дат** (включая прошедшие), но только для привычек, у которых **нет версии** на эту дату. Это заполняет «дыры» в календаре: привычки, к которым не прикасались (не изменяли, не выполняли), теперь отображаются. Удалённые привычки не попадают в фолбэк (их нет в `habits`), они берутся из `habit_versions`. Переименованные привычки имеют версии, поэтому для них используется `habit_versions` с исторически корректным названием.
