...
### 3.3 GetHabitsForDate — разделение по дате

Логика:
- **Сегодня и будущее:** `habits` напрямую (расписание, created_at <= дата).
- **Прошлое:** `habit_versions` + `habit_completions` (привычки с выполнением, но без версии — сироты).
